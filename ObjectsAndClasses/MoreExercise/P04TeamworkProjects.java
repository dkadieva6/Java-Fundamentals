...
public class P04TeamworkProjects {
    public static void main(String[] args) {
        ...
        while (countOfTeams-- > 0) {
            ...
            if (teamsList.stream().anyMatch(e ->
                    e.getName().equals(teamName))) {
                System.out.printf("Team %s was already created!%n", teamName);
            } else if (teamsList.stream().anyMatch(e ->
                    e.getCreator().equals(creator))) {
                System.out.printf("%s cannot create another team!%n", creator);
            } else {
                Team newData = new Team();
                newData.setCreator(input[0]);
                newData.setName(input[1]); teamsList.add(newData);
        ...
        while (!"end of assignment".equals(input = scan.nextLine())) {
            ...
            if (teamsList.stream().noneMatch((e ->
                    e.getName().equals(teamName)))) {
                System.out.printf("Team %s does not exist!%n", teamName);
            } else if ((teamsList.stream().anyMatch(e ->
                    e.getCreator().equals(user)))
                    || (teamsList.stream().flatMap(team ->
                    team.getMembers().stream()).anyMatch(member -> member.equals(user)))) {
        ...
        IntStream.range(0, teamsList.size()).filter(i ->
                !teamsList.get(i).getMembers().isEmpty()).forEach(i -> {
